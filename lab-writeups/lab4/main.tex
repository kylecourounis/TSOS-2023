...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{enumerate}
\item Since VMware is a bare metal hypervisor, that means certain parts of the hardware get directly passed through to the guest OS. If you look at the CPU on a guest OS on VMware, it will list your computer's CPU. You can even pass through graphics cards with a little more work. The amount of RAM you allocate will be passed through (usually dynamically - I've noticed in Activity Monitor/Task Manager that it doesn't eat it all at once), as well as other hardware elements. The host operating system you choose matters when you use something like VMware ESXi, which has its own kernel and is an OS specifically designed for running VMs. 
